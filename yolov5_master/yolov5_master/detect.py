    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        x1, x2 = annotator.box_label(xyxy, label, color=colors(c, True))
                        my_param.x1.append(x1)
                        my_param.x2.append(x2)
            ...
            if view_img:
                ...
                cv2.setMouseCallback(str(p), ON_EVENT_LBUTTONDOWN, my_param)
                if(my_param.client_socket is not None):
                    my_param.client_socket.sendall(generate_camera_values("-1","1", "-9", "1", "0", "-80").encode())
...
def main(opt, connect):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        if connect:
            ...
            my_param.client_socket, client_addr = server_socket.accept()
            print(f"Accepted connection from {client_addr}")
        check_requirements(ROOT / 'requirements.txt', exclude=('tensorboard', 'thop'))
        run(my_param, **vars(opt))
